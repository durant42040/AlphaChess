...
fn test_perft_starting() {
    ...
    let mut engine =
        Engine::from_fen("rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1".to_string());
    for depth in 1..6 {
...
fn test_perft_3() {
    ...
    let mut engine = Engine::from_fen("8/2p5/3p4/KP5r/1R3p1k/8/4P1P1/8 w - - 0 1".to_string());
    for depth in 1..6 {
        let nodes = engine.perft(depth);
        assert_eq!(nodes, results[(depth - 1) as usize]);
    }
...
fn test_perft_5() {
    ...
    let mut engine =
        Engine::from_fen("rnbq1k1r/pp1Pbppp/2p5/8/2B5/8/PPP1NnPP/RNBQK2R w KQ - 1 8".to_string());
    for depth in 1..5 {
        let nodes = engine.perft(depth);
        assert_eq!(nodes, results[(depth - 1) as usize]);
    }
