...
void init_sliding_moves()
{
    ...
    for (int square = 0; square < 64; square++) {
        for (int i = 0; i < (1 << rook_shift_bits[square]); i++) {
            ...
            uint64_t key = (blockers * rook_magic_numbers[square]) >> (64 - rook_shift_bits[square]);
            rook_table[square][key] = generate_rook_moves_slow(Square(square), Bitboard(blockers));
        }
    }
}
...
{
    return king_attacks[from.square_];
}
...
{
    uint64_t blockers = all_pieces.bitboard_ & rook_masks[from.square_].bitboard_;
    uint64_t key = (blockers * rook_magic_numbers[from.square_]) >> (64 - rook_shift_bits[from.square_]);
    return rook_table[from.square_][key];
}

Bitboard generate_queen_moves(Square from, Bitboard all_pieces)
{
    return generate_rook_moves(from, all_pieces) | generate_bishop_moves(from, all_pieces);
}
