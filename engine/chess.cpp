...
void ChessEnv::reset()
{
    if (config::env_chess_initial_fen.size()) {
        ...
        board_ = ChessBoard();
        position_history_.push_back(board_.get_position_info());
        actions_.clear();
...
std::vector<ChessAction> ChessEnv::getLegalActions() const
{
    ...
    for (auto from : board_.our_pieces()) {
        Bitboard moves = board_.generate_legal_moves(from);
...
bool ChessEnv::isLegalAction(const ChessAction& action) const
{
    ...
    return board_.generate_legal_moves(from).get(to) && board_.our_pieces().get(from);
}
...
{
    if (actions_.empty()) {
        return board_.to_string(Square(-1), Square(-1));
    } else {
        return board_.to_string(actions_.back().from_, actions_.back().to_);
    }
}
